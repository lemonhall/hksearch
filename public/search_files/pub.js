...
function fPopUpCalendarDlg(ctrlobj) {
   ...
   var retval = window.showModalDialog("../html/CalendarDlg.htm", "","dialogWidth:197px; dialogHeight:210px; dialogLeft:" + showx +"px; dialogTop:" + showy +"px; status:no; directories:yes;scrollbars:no;Resizable=no; ");
   if (retval != null) {
...
//1.控制键盘输入，只允许数字键录入整数
    function intOnly() {    
	   if (!(((window.event.keyCode >= 48) && (window.event.keyCode <= 57))||(window.event.keyCode == 0x6D))){	
...
//3.控制键盘输入，只允许数字键录入整数或录入'-'号
    function intputTelOnly() {    
	   if ( !(((window.event.keyCode >= 48) && (window.event.keyCode <= 57))||(window.event.keyCode == 0x6D)||(window.event.keyCode==45))){	
			  window.event.keyCode = 0;
			  }
	}
    //判断浏览器
